...
HeaderParser.prototype._parseHeader = function() {
  ...
  for (var i = 0; i < len; ++i) {
    ...
    if (lines[i][0] === '\t' || lines[i][0] === ' ') {
      ...
      // /!\ Modification of the original dicer code start here! /!\
      
      // Original Dicer code:
      //this.header[h][this.header[h].length - 1] += lines[i];
      
      // Modified version, since headers are not arrays but string...
      this.header[h] += lines[i];
      ...
      if (m) {
        ...
        // Modified version, header are never in an array, in case of header duplication, the last overwrite
        this.header[h] = m[2] || '';
